                        ...
                        px_systmr_ticks_t timeout_ticks)
{
    ...
    while(buffer_index < nr_of_bytes)
    {
        ...
        {
            PX_DBG_ERR("RX Timeout (received %u bytes)", buffer_index);
            return false;
        }
    }
...
{
    PX_DBG_INFO("TX Cmd %04X Param %08X", cmd, param);
...
static bool px_gt511_rx_resp_packet(px_systmr_ticks_t timeout_ticks)
{
    ...
    {
        PX_DBG_ERR("RX Header incorrect (%02X %02X %04X)",
    ...
    {
        PX_DBG_ERR("RX Checksum does not match (%04X != %04X)",
    ...
    {
        PX_DBG_ERR("Response field not OK (%02X)", px_gt511_resp_packet.resp);
        return false;
    }
    ...
    if(px_gt511_resp_packet.resp == PX_GT511_CMD_ACK)
    {
        PX_DBG_INFO("RX Resp ACK");
    }
    else
    {
        PX_DBG_INFO("RX Resp NACK (Error Code %04X)", px_gt511_resp_packet.param);
    }
#endif
    return true;
}
                                    ...
                                    px_systmr_ticks_t timeout_ticks)
{
    ...
    // Success
    PX_DBG_INFO("RX Data (%u bytes)", nr_of_bytes);
    return true;
}
