...
static bool px_bme280_cal_rd(void)
{
    ...
    {
        // Error
        PX_DBG_ERR("Unable to read CALIB00 to CALIB25");
        return false;
    }
    ...
    {
        // Error
        PX_DBG_ERR("Unable to read CALIB26 to CALIB32");
        return false;
    }
    // Assemble calibration values
...
static bool px_bme280_temp_raw_rd(int32_t * data)
{
    ...
    {
        // Error
        PX_DBG_ERR("Unable to read temperature");
        return false;
    }
    PX_DBG_INFO("Temp data: %02X %02X %02X", i2c_data[0], i2c_data[1], i2c_data[2]);
    // Assemble value
...
static bool px_bme280_press_raw_rd(int32_t * data)
{
    ...
    {
        // Error
        PX_DBG_ERR("Unable to read pressure");
        return false;
    }
    PX_DBG_INFO("Press data: %02X %02X %02X", i2c_data[0], i2c_data[1], i2c_data[2]);
    // Assemble value
    *data = (int32_t)(   (((uint32_t)i2c_data[0]) << 12)
                       | (((uint32_t)i2c_data[1]) <<  4)
                       | (((uint32_t)i2c_data[2]) >>  4) );
...
static bool px_bme280_hum_raw_rd(int32_t * data)
{
    ...
    {
        // Error
        PX_DBG_ERR("Unable to read humidity");
        return false;
    }
    PX_DBG_INFO("Hum data: %02X %02X", i2c_data[0], i2c_data[1]);
    // Assemble value
