...
static uint8_t px_sd_spi_xc_u8(uint8_t data)
{
    uint8_t data_tx[1];
    uint8_t data_rx[1];
...
{
    return px_sd_spi_xc_u8(0xff);
}

static void px_sd_spi_rd(void *data, size_t nr_of_bytes)
{
...
{
    px_sd_spi_xc_u8(data);
}

static void px_sd_spi_wr(const void * data, size_t nr_of_bytes)
{
    const uint8_t * data_u8 = (uint8_t * )data;
    while(nr_of_bytes != 0)
    {
        px_sd_spi_xc_u8(*data_u8++);
        nr_of_bytes--;
    }
}
...
static uint8_t px_sd_tx_data_block(const uint8_t * data, size_t nr_of_bytes, uint8_t data_token_start)
{
    uint8_t data_token;
...
{
    uint8_t r1, data_token, blocks_read;
    ...
    {
        // Read single block
        if(px_sd_rd_block(data, block_adr))
        {
            return 1;
        }
        else
...
uint8_t px_sd_wr_blocks(const uint8_t * data, uint32_t block_adr, uint8_t nr_of_blocks)
{
    ...
    {
        // Write single block
        if(px_sd_wr_block(data, block_adr))
        {
            return 1;
        }
        else
    ...
    px_sd_spi_xc_u8(PX_SD_TOKEN_DATA_BLOCK_STOP_MULT_WR);
    px_sd_spi_cs_end();
