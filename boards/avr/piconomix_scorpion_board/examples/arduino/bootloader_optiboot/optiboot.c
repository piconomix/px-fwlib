...
int main(void) {
...
#else
  PX_UART_SRA = _BV(U2X0); //Double speed mode USART0
  PX_UART_SRB = _BV(RXEN0) | _BV(TXEN0);
  PX_UART_SRC = _BV(UCSZ00) | _BV(UCSZ01);
  PX_UART_SRL = (uint8_t)( (F_CPU + BAUD_RATE * 4L) / (BAUD_RATE * 8L) - 1 );
#endif
#endif
  ...
  PX_UART_DDR |= _BV(PX_UART_TX_BIT);
#endif
...
void putch(char ch) {
  ...
  __asm__ __volatile__ (
    ...
    :
      [bitcnt] "d" (10),
      [ch] "r" (ch),
      [uartPort] "I" (_SFR_IO_ADDR(PX_UART_PORT)),
      [uartBit] "I" (PX_UART_TX_BIT)
    :
      "r25"
  );
#endif
...
uint8_t getch(void) {
  ...
  __asm__ __volatile__ (
    ...
    :
      [bitCnt] "d" (9),
      [uartPin] "I" (_SFR_IO_ADDR(PX_UART_PIN)),
      [uartBit] "I" (PX_UART_RX_BIT)
    :
      "r25"
);
#else
  while(!(PX_UART_SRA & _BV(RXC0)))
    ;
  if (!(PX_UART_SRA & _BV(FE0))) {
      /*
...
#ifdef SOFT_UART
// AVR305 equation: #define PX_UART_B_VALUE (((F_CPU/BAUD_RATE)-23)/6)
// Adding 3 to numerator simulates nearest rounding for more accurate baud rates
#define PX_UART_B_VALUE (((F_CPU/BAUD_RATE)-20)/6)
#if PX_UART_B_VALUE > 255
