...
/** 
 *  @ingroup AVR
 *  @defgroup AVR_SPI px_spi.h : SPI Peripheral Driver
 *  
 ...
 *  
 *  @warn_s
 *  The SPI peripheral Slave Select pin (SS) must be configured as an output
 ...
 *  from MASTER mode to SLAVE mode.
 *  @warn_e
 *  
 *  @par Example:
...
// Check that all project specific options have been specified in "px_spi_cfg.h"
     ...
     || !defined(PX_SPI_CFG_SPCR              ) \
     || !defined(PX_SPI_CFG_USE_SPI2X         ) \
...
typedef enum
{
    PX_SPI_BAUD_CLK_DIV_2 = 0,       ///< F_CPU / 2
    PX_SPI_BAUD_CLK_DIV_4,           ///< F_CPU / 4
    PX_SPI_BAUD_CLK_DIV_8,           ///< F_CPU / 8
    PX_SPI_BAUD_CLK_DIV_16,          ///< F_CPU / 16
    PX_SPI_BAUD_CLK_DIV_32,          ///< F_CPU / 32
    PX_SPI_BAUD_CLK_DIV_64,          ///< F_CPU / 64
    PX_SPI_BAUD_CLK_DIV_128,         ///< F_CPU / 128
} px_spi_baud_t;
