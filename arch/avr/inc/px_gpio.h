 ...
 *  GPIO pin structure will never change so that it can make the best
 ...
 *      {
 *          // LED is an output pin on PORTB, bit 2, initially off
 *          px_gpio_init(&px_gpio_led, PX_GPIO_LED);
 *          // Initialise LED pin
 *          px_gpio_pin_init(&px_gpio_led);
 *          if(led_on)
 *          {
...
typedef enum
{
    ...
    PX_GPIO_INIT_PULLUP = 1,    ///< Initialize input pin with pull-up enabled
} px_gpio_init_t;
...
typedef struct
{
    ...
    uint8_t        init;        ///< Initial value: PX_GPIO_INIT_HI, PX_GPIO_INIT_LO, PX_GPIO_INIT_HIZ or PX_GPIO_INIT_PULLUP
} px_gpio_handle_t;
 ...
 *  @param init      PX_GPIO_INIT_HI, PX_GPIO_INIT_LO, PX_GPIO_INIT_HIZ or PX_GPIO_INIT_PULLUP
 */
...
#define _GPIO_PIN_SET_HI(port, ddr, pin, bit, dir, init) PX_BIT_SET_HI(*port, bit)

/// Set GPIO pin output low
#define PX_GPIO_PIN_SET_LO(gpio) _GPIO_PIN_SET_LO(gpio)
#define _GPIO_PIN_SET_LO(port, ddr, pin, bit, dir, init) PX_BIT_SET_LO(*port, bit)

/// Toggle GPIO pin output
#define PX_GPIO_PIN_TOGGLE(gpio) _GPIO_PIN_TOGGLE(gpio)
#define _GPIO_PIN_TOGGLE(port, ddr, pin, bit, dir, init) PX_BIT_TOGGLE(*port, bit)
...
#define _GPIO_PIN_IS_HI(port, ddr, pin, bit, dir, init) PX_BIT_IS_HI(*pin, bit)

/// Test if GPIO pin input is low
#define PX_GPIO_PIN_IS_LO(gpio) _GPIO_PIN_IS_LO(gpio)
#define _GPIO_PIN_IS_LO(port, ddr, pin, bit, dir, init) PX_BIT_IS_LO(*pin, bit)
...
#define _GPIO_PULLUP_ENABLE(port, ddr, pin, bit, dir, init) PX_BIT_SET_HI(*port, bit)

/// Disable pull-up on GPIO pin
#define PX_GPIO_PULLUP_DISABLE(gpio) _GPIO_PULLUP_DISABLE(gpio)
#define _GPIO_PULLUP_DISABLE(port, ddr, pin, bit, dir, init) PX_BIT_SET_LO(*port, bit)
...
/// Initialise a GPIO pin
inline void px_gpio_pin_init(const px_gpio_handle_t * gpio)
{
...
/// Set GPIO pin output high
inline void px_gpio_pin_set_hi(const px_gpio_handle_t * gpio)
{
    PX_BIT_SET_HI(*(gpio->port), gpio->bit);
}

/// Set GPIO pin output low
inline void px_gpio_pin_set_lo(const px_gpio_handle_t * gpio)
{
    PX_BIT_SET_LO(*(gpio->port), gpio->bit);
}

/// Toggle GPIO pin output
inline void px_gpio_pin_toggle(const px_gpio_handle_t * gpio)
{
...
/// Test if GPIO pin input is high
inline bool px_gpio_pin_is_hi(const px_gpio_handle_t * gpio)
{
    return PX_BIT_IS_HI(*(gpio->pin), gpio->bit);
}

/// Test if GPIO pin input is low
inline bool px_gpio_pin_is_lo(const px_gpio_handle_t * gpio)
{
...
/// Enable pull-up on GPIO pin
inline void px_gpio_pullup_enable(const px_gpio_handle_t * gpio)
{
    PX_BIT_SET_HI(*(gpio->port), gpio->bit);
}

/// Disable pull-up on GPIO pin
inline void px_gpio_pullup_disable(const px_gpio_handle_t * gpio)
{
    PX_BIT_SET_LO(*(gpio->port), gpio->bit);
}
