/* =============================================================================
     ____    ___    ____    ___    _   _    ___    __  __   ___  __  __ TM
    |  _ \  |_ _|  / ___|  / _ \  | \ | |  / _ \  |  \/  | |_ _| \ \/ /
    | |_) |  | |  | |     | | | | |  \| | | | | | | |\/| |  | |   \  /
    |  __/   | |  | |___  | |_| | | |\  | | |_| | | |  | |  | |   /  \
    |_|     |___|  \____|  \___/  |_| \_|  \___/  |_|  |_| |___| /_/\_\

    Copyright (c) 2020 Pieter Conradie <https://piconomix.com>

    License: MIT
    https://github.com/piconomix/px-fwlib/blob/master/LICENSE.md

    Title:          px_crc8.h : 8-bit CRC calculator
    Author(s):      Pieter Conradie
    Creation Date:  2020-10-23

============================================================================= */

/* _____STANDARD INCLUDES____________________________________________________ */
#include <stdio.h>

/* _____PROJECT INCLUDES_____________________________________________________ */
#include "px_crc8.h"

/* _____LOCAL DEFINITIONS____________________________________________________ */

/* _____MACROS_______________________________________________________________ */

/* _____GLOBAL VARIABLES_____________________________________________________ */

/* _____LOCAL VARIABLES______________________________________________________ */
#if PX_CRC8_RAM_TABLE
static uint8_t px_crc8_table[256];
#endif

#if PX_CRC8_ROM_TABLE
static const uint8_t px_crc8_table[256] =
{
    ,
};
#endif

/* _____LOCAL FUNCTION DECLARATIONS__________________________________________ */

/* _____LOCAL FUNCTIONS______________________________________________________ */

/* _____GLOBAL FUNCTIONS_____________________________________________________ */
void px_crc8_init(void)
{
#if PX_CRC8_RAM_TABLE
    uint8_t  i;
    uint16_t j;
    uint8_t  c;

    for (j = 0; j < 256; j++)
	{
		c = (uint8_t)j;
		// Repeat for each bit in byte
		for (i = 8; i != 0; i--)
		{
			// Most Significant Bit set?
			if (c & 0x80)
			{
				c = (c << 1) ^ PX_CRC8_POLYNOMIAL;
			}
			else
			{
				c = (c << 1);
			}
		}
		px_crc8_table[j] = c;
	}
#endif
}

uint8_t px_crc8_update_u8(uint8_t crc, uint8_t data)
{
#if PX_CRC8_RAM_TABLE || PX_CRC8_ROM_TABLE
    crc = px_crc8_table[crc ^ data];
#else
    uint8_t i;

    crc ^= data;
    for(i=8; i != 0; i--)
    {
        if(crc & 0x80)
        {
            crc = (crc << 1) ^ PX_CRC8_POLYNOMIAL;
        }
        else
        {
            crc = (crc << 1);
        }
    }

#endif
    return crc;
}

uint8_t px_crc8_update_data(uint8_t crc, const void * data, size_t nr_of_bytes)
{
    const uint8_t * data_u8 = (uint8_t *)data;

    while(nr_of_bytes)
    {
        crc = px_crc8_update_u8(crc, *data_u8++);
        nr_of_bytes--;
    }

    return crc;
}
